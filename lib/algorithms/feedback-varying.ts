...
export function feedbackVaryingScheduling(
  processes: Process[],
  numberOfQueues = 3,
  quantumMultiplier = 2,
): SchedulingResult {
  ...
  while (processIndex < processQueue.length || queues.some((q) => q.length > 0)) {
    ...
    // Variable time quantum: base * multiplier^queueLevel
    const baseQuantum = 1
    const timeQuantum = baseQuantum * Math.pow(quantumMultiplier, currentQueueIndex)
